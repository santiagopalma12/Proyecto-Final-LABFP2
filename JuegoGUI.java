...
public class JuegoGUI extends JFrame {
    ...
    private JButton btnAtacar, btnSeleccionarCriatura, btnCapturarPokemon, btnNuevoCombate;
    ...
    public JuegoGUI() {
        ...
        // Crear y configurar el botón Nuevo Combate
        ...
        // Configurar la acción del botón Nuevo Combate
        btnNuevoCombate.addActionListener(e -> iniciarNuevoCombate());
        actualizarTexto();
        actualizarImagenes();
    }
    public class ImageUtil {
        public static ImageIcon escalarImagen(ImageIcon iconoOriginal, int anchoNuevo, int altoNuevo) {
    ...
    private void reiniciarJuego() {
        ...
        entrenadorRival.getEquipo().clear();
    
        // Volver a seleccionar equipos
        seleccionarEquipoInicial();
        crearEntrenadorRival();
    
        // Restablecer el enemigo inicial
        enemigo = entrenadorRival.getEquipo().get(0);
        criaturaSeleccionada = null;
    
        // Actualizar texto e imágenes
        textArea.setText(""); // Limpiar el área de texto
        actualizarTexto();
        actualizarImagenes();
    
        textArea.append("¡El juego se ha reiniciado! Selecciona una criatura para comenzar.\n");
    }
    ...
    private void mostrarOpcionReintentar() {
        int opcion = JOptionPane.showOptionDialog(
            this,
            "¿Deseas luchar otra vez o salir del juego?",
            "Fin de la Batalla",
            JOptionPane.YES_NO_OPTION,
            JOptionPane.QUESTION_MESSAGE,
            null,
            new String[]{"Luchar otra vez", "Salir"},
            "Luchar otra vez"
        );
    ...
    private JButton crearBotonPocion(int index, boolean esJugador) {
        ...
        // No deshabilitar botones del enemigo para permitir representación gráfica.
        return boton;
    }
    ...
    private void turnoEnemigo() {
        // Si el Pokémon del enemigo está derrotado, eliminamos al Pokémon y seleccionamos uno nuevo
        if (enemigo.getSalud() <= 0) {
            textArea.append("\n" + enemigo.getNombre() + " ha sido derrotado.\n");
            entrenadorRival.getEquipo().remove(enemigo);
    
            if (entrenadorRival.getEquipo().isEmpty()) {
                textArea.append("¡Has ganado el combate! No quedan Pokémon enemigos.\n");
                mostrarOpcionReintentar();
                return; // Fin del combate
            }

            if(entrenador1.getEquipo().isEmpty()){
                textArea.append("¡Has perdido el combate! No quedan más Pokémon en tu equipo.\\n");
                mostrarOpcionReintentar();
                return;
            }
    
            enemigo = entrenadorRival.getEquipo().get(0); // El siguiente Pokémon enemigo
            textArea.append("El próximo Pokémon enemigo es: " + enemigo.getNombre() + ".\n");
        }
    
        // Si el Pokémon del jugador está derrotado, lo eliminamos y elegimos otro
        if (criaturaSeleccionada.getSalud() <= 0) {
            textArea.append("\n" + criaturaSeleccionada.getNombre() + " ha sido derrotado.\n");
            entrenador1.getEquipo().remove(criaturaSeleccionada);
    
            if (entrenador1.getEquipo().isEmpty()) {
                textArea.append("¡Has perdido el combate! No quedan Pokémon en tu equipo.\n");
                return; // Fin del combate
            }
    
            textArea.append("Selecciona otro Pokémon para continuar.\n");
            seleccionarCriatura(); // Seleccionar un nuevo Pokémon
            return;
        }
    
        // Intentar usar una poción antes de atacar
        if (entrenadorRival.getPocionesCuracion() > 0) {
            boolean pocionUsada = usarPocionRival();
            if (pocionUsada) {
                actualizarTexto();
                return; // Terminar turno si se usa una poción
            }
        }
    
        // Si no se usó una poción, el enemigo ataca
    ...
    private boolean usarPocionRival() {
        // Encontrar al Pokémon del equipo rival con la salud más baja
        Criatura criaturaCurada = entrenadorRival.getEquipo().stream()
            .filter(c -> c.getSalud() > 0 && c.getSalud() < c.getSaludMaxima()) // Salud menor a la máxima
            .min(Comparator.comparingInt(Criatura::getSalud)) // Seleccionar el de menor salud
            .orElse(null);
        if (criaturaCurada != null) {
            // Usar poción para curar al Pokémon seleccionado
            criaturaCurada.curar(20);
            entrenadorRival.restarPocionesCuracion(1);
            textArea.append("\nEl enemigo ha usado una poción de curación.\n");
            textArea.append("El rival curó a " + criaturaCurada.getNombre() +
                            ". Salud actual: " + criaturaCurada.getSalud() + "\n");
            // Actualizar interfaz gráfica
            for (JButton boton : pocionesEnemigo) {
                if (boton.isEnabled()) {
                    boton.setIcon(new ImageIcon("Imagenes/pocion_usada.png")); // Imagen de poción usada
                    boton.setEnabled(false); // Desactivar botón
                    break;
                }
            }
            return true; // Indicar que se usó una poción
        }
